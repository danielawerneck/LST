...
# User Requirements
# --------------------------------------------------
#MODIFICADO - modificado para alterar os parâmetros
select_parameters = ['LST','EVI','NDBI']
select_metrics = ['mean']
percentiles = []
...
# Space
select_roi = ee.Geometry.Rectangle([-48.26, -15.41, -47.46, -16.05])
max_cloud_cover = 20
epsg = 'EPSG:31983'
...
#MODIFICADO - Enhanced Vegetation Index
def fun_evi(img):
    evi = img.expression(
...
#MODIFICADO - Normalized Difference Built-In Index
def fun_ndbi(img):
    ndbi = img.expression(
                         '(SWIR-NIR)/(SWIR+NIR)',
                         {
...
#MODIFICADO - Normalized Difference Moisture Index
def fun_ndmi(img):
    ndmi = img.expression(
                         '(NIR-SWIR)/(SWIR+NIR)',
                         {
...
#MODIFICADO - Imprime metadados das imagens de uma collection 
def fun_printInfo(imgCol):
    #Determina o numero de imagens na collection
    numImg = imgCol.size().getInfo()
    print('Numero de Imagens nesta collection: ', numImg, '\n')

    #Cria uma lista de imagens a partir da collection
    imgList = imgCol.toList(numImg)
    #Imprime o Asset ID e Cloud Cover de cada uma das imagens na collection
    for i in range(numImg):
        img = ee.Image(imgList.get(i))
        assetID = img.get('system:id').getInfo()
        cldCovr = img.get('CLOUD_COVER').getInfo()
        print('ID: ', assetID, 'Cloud Cover: ', cldCovr)
    return
...
#MODIFICADO - Imprime no prompt os metadados de imgCol_L7_SR (comentado pois não estamos utilizando L7 no momento)
#fun_printInfo(imgCol_L7_SR)
...
#MODIFICADO - Imprime no prompt os metadados de imgCol_L8_SR
fun_printInfo(imgCol_L8_SR)
...
# Merge Collections
#MODIFICADO (removendo Landsat 5 e 7)
...
#MODIFICADO
imgCol_merge = imgCol_merge.map(fun_ndbi)
#MODIFICADO
imgCol_merge = imgCol_merge.map(fun_ndmi)
...
for parameter in select_parameters:
    ...
    #MODIFICADO - Imprime o numero de imagens em subCol
    numImg = subCol.size().getInfo()
    print('Numero de Imagens na collection principal: ', numImg, '\n')
    ...
    for metric in select_metrics:
        if metric == 'ts':
            ...
            if (parameter == 'LST') or (parameter == 'LST_USGS'):
...
#MODIFICADO
print('\nTotal de arquivos salvos no Drive: ' + str(cont) + '. Fim da Execução.')
